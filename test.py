...
def record_audio(ask=False):
    with sr.Microphone() as source:  # microphone as source
        ...
        try:
            ...
            speak('I did not get that, the recognizer does not understand')
        except sr.RequestError:
            speak('Sorry, the service is down, recognizer not connected')  # error: recognizer is not connected
...
def speak(audio_string):
    ...
    l1 = tk.Label(master, text=f"Simba: {audio_string}", bg="black", fg="white")
...
def respond(voice_data):
    ...
    # 1: greeting
    if there_exists(['hey', 'hi', 'hello', "Simba"]):
    ...
    # 2: name
        ...
        if person_obj.name:
            speak("my name is Simba")
        else:
            speak("my name is Simba. what's your name?")
